...
const TECHNICAL_NOTES={provider_unavailable:"The ChatGPT browser had a temporary outage on the last try. The attempt was refunded and retries automatically — no result was lost."};
const technicalNote=value=>{const text=String(value||"");if(TECHNICAL_NOTES[text])return TECHNICAL_NOTES[text];if(text.startsWith("infrastructure_budget_exhausted"))return "The connection to ChatGPT failed repeatedly, so this problem was paused without a mathematical attempt. It is requeued automatically at the next pipeline launch.";return text};
