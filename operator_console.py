...
def _default_config(root: Path = ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        # A completed response before this wall-clock horizon triggers another
        # transcript-bound research wave. This is active model work, not sleep.
        "minimum_reasoning_seconds": 7200,
        "free_reasoning": True,
...
def _save_config(config: dict[str, Any], root: Path = ROOT) -> None:
    ...
    clean["budget"] = budget
    minimum_reasoning = float(clean.get("minimum_reasoning_seconds", 7200))
    if not math.isfinite(minimum_reasoning) or not 0 <= minimum_reasoning <= 36000:
        raise OperatorError(
            "minimum_reasoning_seconds must be between 0 and 36000")
    clean["minimum_reasoning_seconds"] = minimum_reasoning
...
class Operator:
    ...
    def _start(self) -> str:
        ...
        env["EGMRA_MINIMUM_REASONING_SECONDS"] = str(
            int(config.get("minimum_reasoning_seconds", 7200)))
