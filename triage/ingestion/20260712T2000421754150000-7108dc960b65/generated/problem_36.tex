...
The minimum overlap problem. The example (with $N$ even) $A=\{N/2+1,\ldots,3N/2\}$ shows that $c\leq 1/2$ (indeed, Erd\H{o}s initially conjectured that $c=1/2$). The lower bound of $c\geq 1/4$ is trivial, and Scherk improved this to $1-1/\sqrt{2}=0.29\cdots$. The current records are\[0.379005 < c < 0.380876,\]the lower bound due to White \cite{Wh22} and the upper bound due to the TTT-Discover LLM \cite{YKLBMWKCZGS26}, improving slightly on earlier bounds due to AlphaEvolve \cite{GGTW25} and Haugland \cite{Ha16}.
